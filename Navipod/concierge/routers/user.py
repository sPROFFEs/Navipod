...
import json
import database, auth, os, shutil
...
async def upload_avatar(
    ...
    dl_settings = db.query(database.DownloadSettings).filter(
        database.DownloadSettings.user_id == user.id
    ).first()
    ...
    try:
        ...
        try:
            ...
            return templates.TemplateResponse("user_settings.html", {
                "request": request, "user": user,
                "error": f"Failed to process image: Invalid or corrupt file",
                "is_admin": user.is_admin, "username": user.username,
                "dl_settings": dl_settings
            })
