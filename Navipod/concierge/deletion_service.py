...
def detach_playlist_references(db: Session, playlist_id: int) -> None:
    """Remove owned items while retaining download history without a dead target."""
...
def detach_track_references(db: Session, track_id: int) -> None:
    """Apply the lifecycle policy for every application reference to a track."""
    ...
    party_service.remove_track_from_queues(db, track_id)
